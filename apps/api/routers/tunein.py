...
#     user_id = verify_token(credentials.credentials)
#     svc     = get_scoring_service()
#     return await svc.get_user_dashboard(user_id)

# @router.get("/leaderboard")
# async def get_leaderboard(top_k: int = 50, credentials: HTTPAuthorizationCredentials = Depends(security)):
#     user_id = verify_token(credentials.credentials)
#     svc     = get_scoring_service()
#     return await svc.get_leaderboard(top_k=top_k, requesting_user_id=user_id)

# @router.get("/user-activity/{target_user_id}")
# async def get_user_activity(target_user_id: str, credentials: HTTPAuthorizationCredentials = Depends(security)):
#     verify_token(credentials.credentials)
#     svc = get_scoring_service()
#     return svc.get_user_activity(target_user_id)

# @router.post("/invalidate-cache")
# async def invalidate_cache(credentials: HTTPAuthorizationCredentials = Depends(security)):
#     verify_token(credentials.credentials)
#     svc = get_scoring_service()
#     svc.invalidate_scores_cache()
#     return {"ok": True}