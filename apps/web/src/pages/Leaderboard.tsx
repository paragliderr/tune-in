...
// ─── Drop-in replacement for fetchOverallLeaderboard in Leaderboard.tsx ───
// Pass this the session token so the /api/tune-in/leaderboard endpoint
// receives the required Authorization header.

async function fetchOverallLeaderboard(): Promise<LeaderboardEntry[]> {
  try {
    const [cinema, anime, gaming, fitness, tech, { data: profiles }, sessionResult] =
      await Promise.all([
    ...
    // ── HGT backend call with auth token ──────────────────────────────────
    ...
    if (token) {
      try {
        ...
        if (res.ok) {
          ...
          console.warn(`[Leaderboard] HGT backend returned ${res.status} — skipping diversity multiplier`);
        }
      } catch (err) {
        console.warn("[Leaderboard] HGT backend unreachable — skipping diversity multiplier", err);
      }
    } else {
      console.warn("[Leaderboard] No session token — skipping HGT diversity multiplier");
    }
    // ──────────────────────────────────────────────────────────────────────

    const getScore = (arr: LeaderboardEntry[], id: string) =>
      arr.find(x => x.user_id === id)?.score || 0;

    // Normalise each category to a 0-100 scale using the top scorer
    const cinemaMax  = cinema[0]?.score  || 1;
    const animeMax   = anime[0]?.score   || 1;
    const gamingMax  = gaming[0]?.score  || 1;
    const fitnessMax = fitness[0]?.score || 1;
    const techMax    = tech[0]?.score    || 1;
    ...
    return (profiles ?? []).map(p => {
      ...
      const normalised = [
        (cinemaScore  / cinemaMax)  * 100,
        (animeScore   / animeMax)   * 100,
        (gamingScore  / gamingMax)  * 100,
        (fitnessScore / fitnessMax) * 100,
        (techScore    / techMax)    * 100,
      ];

      const avg = normalised.reduce((a, b) => a + b, 0) / normalised.length;

      // HGT graph diversity multiplier (defaults to 1.0 if backend unavailable)
...
// Gaming: steam_stats score + gaming club membership bonus (15pts each)
async function fetchGamingLeaderboard(): Promise<LeaderboardEntry[]> {
  try {
    const [{ data: profiles }, { data: steam }, { data: clubs }] = await Promise.all([
      supabase.from("profiles").select("id, username, avatar_url"),
      supabase.from("steam_stats").select("user_id, score"),
      supabase.from("club_members").select("user_id, clubs!inner(name)").ilike("clubs.name", "%gaming%")
    ]);

    const scoreMap: Record<string, number> = {};

    // Use the pre-computed score from steam_stats directly
    steam?.forEach(s => {
      scoreMap[s.user_id] = (scoreMap[s.user_id] || 0) + (s.score || 0);
    });

    // Bonus for being in gaming clubs
    clubs?.forEach(c => {
      scoreMap[c.user_id] = (scoreMap[c.user_id] || 0) + 15.0;
    });
    ...
    console.error("Gaming fetch failed:", error);
    return [];
  }
}

// Fitness: strava_stats score + fitness club membership bonus (15pts each)
async function fetchFitnessLeaderboard(): Promise<LeaderboardEntry[]> {
  try {
    const [{ data: profiles }, { data: strava }, { data: clubs }] = await Promise.all([
      supabase.from("profiles").select("id, username, avatar_url"),
      supabase.from("strava_stats").select("user_id, score"),
      supabase.from("club_members").select("user_id, clubs!inner(name)").ilike("clubs.name", "%fitness%")
    ]);

    const scoreMap: Record<string, number> = {};

    // Use the pre-computed score from strava_stats directly
    strava?.forEach(s => {
      scoreMap[s.user_id] = (scoreMap[s.user_id] || 0) + (s.score || 0);
    });

    // Bonus for being in fitness clubs
    clubs?.forEach(c => {
      scoreMap[c.user_id] = (scoreMap[c.user_id] || 0) + 15.0;
    });
    ...
    console.error("Fitness fetch failed:", error);
    return [];
  }
}

// Tech: GitHub commits + streak bonus + tech club membership bonus
async function fetchTechLeaderboard(): Promise<LeaderboardEntry[]> {
  try {
    ...
    github?.forEach(g => {
      scoreMap[g.user_id] = (scoreMap[g.user_id] || 0)
        + (g.total_commits || 0) * 1.0
        + (g.streak_days   || 0) * 5.0;
    });
    ...
    return (profiles ?? []).map(p => ({
      ...
      breakdown:  { tech: scoreMap[p.id] ?? 0 }
    })).filter(e => e.score > 0).sort((a, b) => b.score - a.score).slice(0, 50);
  } catch (error) {
    console.error("Tech fetch failed:", error);
    return [];
  }
}
                  ...
                  {entries.map((entry, idx) => {
                    ...
                    return (
                      <motion.div
                        ...
                        className={`flex items-center gap-4 px-5 py-3.5 hover:bg-muted/40 cursor-pointer transition-colors ${isMe ? "bg-primary/5" : ""}`}
                      >
...
const PodiumCard = ({
  ...
  entry: LeaderboardEntry; rank: number; height: string;
  navigate: ReturnType<typeof useNavigate>; highlight?: boolean;
}) => {
  const safeName = entry.username || "User";
  return (
    <div
    ...
    >
      <div className={`relative ${rank === 1 ? "ring-2 ring-yellow-400/60 rounded-full" : ""}`}>
        {entry.avatar_url ? (
          <img
            src={entry.avatar_url} alt={safeName}
            className={`${rank === 1 ? "w-14 h-14" : "w-10 h-10"} rounded-full object-cover`}
          />
        ) : (
...
const BreakdownLine = ({ entry, category }: { entry: LeaderboardEntry; category: CategoryKey }) => {
  ...
  if (category === "overall") {
    ...
    if (breakdown.tech)    parts.push(`${breakdown.tech} pts`);
  } else if (category === "fitness") {
    if (breakdown.fitness) parts.push(`${breakdown.fitness} pts`);
  } else if (category === "gaming") {
    if (breakdown.games)   parts.push(`${breakdown.games} pts`);
  } else {
...
const ScoringNote = ({ category }: { category: CategoryKey }) => {
  const notes: Record<CategoryKey, string> = {
    overall:  "Normalised avg across all categories × HGT Graph Diversity Multiplier",
    cinema:   "Score = total movies reviewed",
    anime:    "Score = total anime reviewed",
    gaming:   "Score = Steam score + (Gaming clubs × 15)",
    tech:     "Score = GitHub commits + (streak × 5) + (Tech clubs × 15)",
    fitness:  "Score = Strava score + (Fitness clubs × 15)",
  };
  return (
